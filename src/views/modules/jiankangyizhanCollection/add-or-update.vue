    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="jiankangyizhanCollectionTypes">
                        <el-select v-model="ruleForm.jiankangyizhanCollectionTypes" :disabled="ro.jiankangyizhanCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in jiankangyizhanCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="jiankangyizhanCollectionValue">
                        <el-input v-model="ruleForm.jiankangyizhanCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    yonghuId: false,
                    jiankangyizhanCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    jiankangyizhanId: '',
                    yonghuId: '',
                    jiankangyizhanCollectionTypes: '',
                    insertTime: '',
                },
                jiankangyizhanCollectionTypesOptions : [],
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiankangyizhan_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.jiankangyizhanCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `jiankangyizhanCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`jiankangyizhanCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.jiankangyizhanCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
